...
-- ============================================
-- 9. 用户生词本表 (user_vocabulary)
-- ============================================
CREATE TABLE user_vocabulary (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id VARCHAR(100) NOT NULL,  -- 用户ID（登录用户）
    word VARCHAR(255) NOT NULL,      -- 单词
    phonetic VARCHAR(255),           -- 音标
    meaning TEXT,                    -- 含义/翻译
    part_of_speech VARCHAR(50),      -- 词性 (noun, verb, adj, etc.)
    sentence_context TEXT,           -- 例句上下文
    source_sentence_id BIGINT,       -- 来源句子ID (可选，关联sentences表)
    source_article_id BIGINT,        -- 来源文章ID (可选，关联articles表)
    notes TEXT,                      -- 用户笔记
...
CREATE INDEX idx_user_vocabulary_next_review ON user_vocabulary(next_review_at) WHERE next_review_at IS NOT NULL;
CREATE INDEX idx_user_vocabulary_created_at ON user_vocabulary(created_at DESC);

-- 唯一约束：同一用户不能重复添加相同单词
CREATE UNIQUE INDEX idx_user_vocabulary_user_word_unique 
    ON user_vocabulary(user_id, LOWER(word));

-- ============================================
-- 10. RLS 策略 - 生词本（仅登录用户可访问）
-- ============================================
...
-- SELECT * FROM information_schema.columns WHERE table_name = 'user_vocabulary' ORDER BY ordinal_position;
