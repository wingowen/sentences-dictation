...
export async function handler(event, context) {
  try {
    ...
    // Build sentences array - each sentence gets the FULL Chinese paragraph
    // (website doesn't provide 1:1 sentence translations, so show full context)
    const sentences = englishSentences.map((en) => ({
      text: en,
      translation: chineseParagraph.trim()
    }));

    console.log(`Extracted ${sentences.length} sentences with full Chinese paragraph`);
