...
export async function handler(event, context) {
  ...
  try {
    console.log('[get-new-concept-3] 开始获取数据...');

    const cachedData = readCache('get-new-concept-3', {});
    if (cachedData) {
      console.log('[get-new-concept-3] 使用缓存数据');
      return { statusCode: 200, body: JSON.stringify(cachedData), headers: CORS_HEADERS };
    }

    const dataPath = getDataFilePath('new-concept-3.json');
    console.log(`[get-new-concept-3] 读取文件: ${dataPath}`);
    ...
    if (localData && localData.success && localData.articles && localData.articles.length > 0) {
      console.log(`[get-new-concept-3] 成功读取 ${localData.articles.length} 篇课文`);
      ...
      console.log('[get-new-concept-3] 数据已缓存');

      return { statusCode: 200, body: JSON.stringify(result), headers: CORS_HEADERS };
    }

    console.error('[get-new-concept-3] 本地数据格式不正确或为空');
    return {
      statusCode: 404,
      body: JSON.stringify({
        success: false,
    ...
    console.error('[get-new-concept-3] 错误:', error.message);
    return {
      statusCode: 500,
      body: JSON.stringify({ success: false, error: error.message || 'Failed to fetch New Concept English 3 articles' }),
      headers: CORS_HEADERS
    };
  }
}
